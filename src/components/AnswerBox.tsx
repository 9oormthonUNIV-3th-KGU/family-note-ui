...
const AnswerBox: React.FC<AnswerBoxProps> = ({ content, id }) => {
  ...
  const fetchData = async () => {
    ...
    if (currentQuestion?.id) {
      const data = await FetchFamilyAnswers(currentQuestion?.id)
  ...
  useEffect(() => {
    fetchData()
    console.log('answer-current: ', currentQuestion?.id)
    console.log('Current display state:', isDisplayed)
  }, [id, isDisplayed, currentQuestion?.id])
  ...
  return (
    <>
      {currentQuestion?.isAnswerVisible && (
        <Box
